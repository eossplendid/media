...
/* Merge pipe_new into pipe_old when both target same mix bus.
   Extracts source nodes (no incoming links) from both, builds: srcA, srcB, ... -> mixer -> sink.
   Supports simple topologies; intermediate nodes (resampler etc) are preserved per source chain. */
static int merge_pipelines_for_mix(session_t *session, pipeline_t *pipe_old, pipeline_t *pipe_new) {
    pipeline_id_t keep_id = pipeline_get_id(pipe_old);
    const char *mt = pipeline_get_mix_target(pipe_old);

    /* Find sink config from pipe_old (last node with no outgoing links) */
    int sink_idx = -1;
    ...
    for (int i = 0; i < pipe_old->num_nodes; i++) {
        ...
        for (int j = 0; j < pipe_old->num_links; j++) {
            ...
            if (strcmp(pipe_old->links[j].from_id, pipe_old->node_ids[i]) == 0) { has_out = 1; break; }
        }
        if (!has_out) { sink_idx = i; spk_cfg = pipe_old->node_configs[i]; break; }
    }

    /* Collect feeder node ids (nodes that link directly to sink) for each pipeline */
    ...
    for (int i = 0; i < pipe_new->num_nodes; i++) {
        ...
        for (int j = 0; j < pipe_new->num_links; j++) {
            ...
            if (strcmp(pipe_new->links[j].from_id, pipe_new->node_ids[i]) == 0) { has_out = 1; break; }
        }
        if (!has_out) { sink_id_new = pipe_new->node_ids[i]; break; }
    }
    ...
    for (int i = 0; i < pipe_old->num_nodes; i++) {
        ...
        if (sink_id_old && strcmp(pipe_old->node_ids[i], sink_id_old) == 0) continue;
        char aid[80];
        snprintf(aid, sizeof(aid), "a_%s", pipe_old->node_ids[i]);
        pipeline_add_node(merged, n->desc->name, aid, &pipe_old->node_configs[i]);
    }
    for (int i = 0; i < pipe_old->num_links; i++) {
    ...
    for (int i = 0; i < pipe_new->num_nodes; i++) {
        ...
        if (!n || !n->desc) continue;
        if (sink_id_new && strcmp(pipe_new->node_ids[i], sink_id_new) == 0) continue;
        char bid[80];
        snprintf(bid, sizeof(bid), "b_%s", pipe_new->node_ids[i]);
        pipeline_add_node(merged, n->desc->name, bid, &pipe_new->node_configs[i]);
    }
    for (int i = 0; i < pipe_new->num_links; i++) {
    ...
    node_config_t mix_cfg = { .input_count = (uint32_t)mix_inputs, .sample_rate = 48000 };
